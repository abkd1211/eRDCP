...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (isAllowed) {
      ...
      console.warn(`[CORS REJECTED] AI Agent Service - Origin: "${origin}" mismatched. Allowed: ${allowed.join(', ')}`);
      callback(null, false);
    }
  },
...
// ─── Root Metadata ────────────────────────────────────────────────────────────
app.get('/', (_req, res) => {
  res.json({ status: 'active', service: env.SERVICE_NAME, info: 'Use /agent for API endpoints' });
});
...
app.get('/agent/swagger.yaml', (_req, res) => {
